...
for i in range(len(inp)):
    ...
    bar.update(30);
    os.system("pdflatex %s >> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(Track\).tex'));
    bar.update(35);
    os.system("bibtex %s >> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(Track\).aux'));
    bar.update(40);
    os.system("pdflatex %s >> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(Track\).tex'));
    bar.update(45);
    os.system("pdflatex %s >> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(Track\).tex'));
    bar.update(50);
    os.system("rm %s 2> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(Track\).bbl'));
    os.system("rm %s 2> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(Track\).aux'));
    os.system("rm %s 2> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(Track\).blg'));
    os.system("rm %s 2> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(Track\).log'));
    os.system("rm %s 2> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(Track\).out'));
    os.system("rm %s 2> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(Track\)Notes.bib'));

    bar.update(55);
    os.system("pdflatex %s >> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(FullTrack\).tex'));
    bar.update(60);
    os.system("bibtex %s >> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(FullTrack\).aux'));
    bar.update(65);
    os.system("pdflatex %s >> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(FullTrack\).tex'));
    bar.update(70);
    os.system("pdflatex %s >> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(FullTrack\).tex'));
    bar.update(75);
    os.system("rm %s 2> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(FullTrack\).bbl'));
    os.system("rm %s 2> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(FullTrack\).aux'));
    os.system("rm %s 2> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(FullTrack\).blg'));
    os.system("rm %s 2> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(FullTrack\).log'));
    os.system("rm %s 2> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(FullTrack\).out'));
    os.system("rm %s 2> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','\(FullTrack\)Notes.bib'));

    bar.update(80);
    os.system("pdflatex %s >> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','.tex'));
    bar.update(85);
    os.system("bibtex %s >> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','.aux'));
    bar.update(90);
    os.system("pdflatex %s >> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','.tex'));
    bar.update(95);
    os.system("pdflatex %s >> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','.tex'));
    bar.update(99);
    os.system("rm %s 2> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','.aux'));
    os.system("rm %s 2> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','.blg'));
    os.system("rm %s 2> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','.log'));
    os.system("rm %s 2> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','.out'));
    os.system("rm %s 2> /dev/null" % files[int(inp[i]) - 1].replace('(Track).tex','Notes.bib'));

    fileread = open(files[int(inp[i]) - 1].replace('(Track).tex','.bbl'), 'r');
    ReadLines = fileread.readlines();
    Delbbl = False;
    for Line in ReadLines:
        if Line == '\\begin{thebibliography}{0}%\n':
            Delbbl = True;
    fileread.close();
    if Delbbl == True:
        os.system("rm %s" % files[int(inp[i]) - 1].replace('(Track).tex','.bbl'));
    bar.update(100);
    bar.finish();
    print();
print("Done!\n");
